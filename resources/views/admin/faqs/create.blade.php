...
@section('title', __('admin.faqs.add_new'))
@section('page_title', __('admin.faqs.add_new'))

@section('content')
<div class="max-w-4xl space-y-6" x-data="{ langTab: 'en' }">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-extrabold text-slate-900 dark:text-white tracking-tight">{{ __('admin.faqs.add_new') }}</h1>
    ...
    <form method="POST" action="{{ route('admin.faqs.store') }}" class="bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 sm:p-8 shadow-xl space-y-6">
        @csrf
        ...
        <div x-show="langTab === 'en'" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" 
                       ...
                       required 
                       dir="ltr"
                       placeholder="e.g. How does your team handle enterprise IP protection?" 
            ...
            <div>
                ...
                <textarea id="answer_en" 
                          ...
                          required 
                          dir="ltr"
                          placeholder="Comprehensive, reassuring explanation covering NDAs, escrow, and code ownership..." 
                          class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-950/60 border border-slate-300 dark:border-slate-800 rounded-xl text-slate-900 dark:text-white text-sm focus:ring-2 focus:ring-indigo-500">{{ old('answer.en') }}</textarea>
            </div>
            ...
            <div>
                ...
                <input type="text" 
                       id="category_en" 
                       name="category[en]" 
                       value="{{ old('category.en') }}" 
                       dir="ltr"
                       placeholder="e.g. Security & IP" 
                       class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-950/60 border border-slate-300 dark:border-slate-800 rounded-xl text-slate-900 dark:text-white text-sm focus:ring-2 focus:ring-indigo-500">
            </div>
        </div>
        ...
        <div x-show="langTab === 'ar'" x-cloak class="space-y-4">
            ...
            <div>
                ...
                <input type="text" 
                       ...
                       required 
                       dir="rtl"
                       placeholder="مثال: كيف تضمنون حماية الملكية الفكرية والسرية للشركات؟" 
                       class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-950/60 border border-slate-300 dark:border-slate-800 rounded-xl text-slate-900 dark:text-white text-sm focus:ring-2 focus:ring-indigo-500">
            </div>
            ...
            <div>
                ...
                <textarea id="answer_ar" 
                          ...
                          required 
                          dir="rtl"
                          placeholder="إجابة وافية تغطي اتفاقيات عدم الإفصاح والملكية الكاملة للشفرة..." 
                          class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-950/60 border border-slate-300 dark:border-slate-800 rounded-xl text-slate-900 dark:text-white text-sm focus:ring-2 focus:ring-indigo-500">{{ old('answer.ar') }}</textarea>
            </div>
            ...
            <div>
                ...
                <input type="text" 
                       id="category_ar" 
                       name="category[ar]" 
                       value="{{ old('category.ar') }}" 
                       dir="rtl"
                       placeholder="مثال: الأمان والملكية الفكرية" 
                       class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-950/60 border border-slate-300 dark:border-slate-800 rounded-xl text-slate-900 dark:text-white text-sm focus:ring-2 focus:ring-indigo-500">
            </div>
        </div>
