...
<div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-extrabold text-white tracking-tight">Frequently Asked Questions (FAQs)</h1>
            <p class="text-sm text-slate-400 mt-1">Manage executive inquiries, SLAs, engagement security, and billing answers.</p>
        </div>
    ...
    <div class="bg-slate-900/80 border border-slate-800 rounded-2xl p-4 shadow-xl">
        <form method="GET" action="{{ route('admin.faqs.index') }}" class="grid grid-cols-1 sm:grid-cols-12 gap-3">
            ...
            <div class="sm:col-span-3">
                <select name="category" class="w-full px-3 py-2 bg-slate-950/60 border border-slate-800 rounded-xl text-white text-sm focus:ring-2 focus:ring-indigo-500">
    ...
    <div class="bg-slate-900/80 border border-slate-800 rounded-2xl overflow-hidden shadow-xl">
                    ...
                    <tbody class="divide-y divide-slate-800/60">
                        @foreach($faqs as $faq)
                            <tr class="hover:bg-slate-800/30 transition">
                                ...
                                <td class="px-6 py-4">
                                    <div>
                                        <a href="{{ route('admin.faqs.edit', $faq) }}" class="font-semibold text-white hover:text-indigo-400 transition">
                                            {{ $faq->question }}
                                        </a>
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        <a href="{{ route('admin.faqs.edit', $faq) }}" class="p-1.5 text-slate-400 hover:text-indigo-400 hover:bg-slate-800 rounded-lg transition" title="Edit">
        ...
        @else
            <div class="py-16 px-4 text-center">
                ...
                <h3 class="text-base font-bold text-white">No FAQs Registered</h3>
                <p class="text-xs text-slate-400 mt-1 max-w-sm mx-auto">Address recurring client questions on process, architecture, and security.</p>
                <div class="mt-4">
