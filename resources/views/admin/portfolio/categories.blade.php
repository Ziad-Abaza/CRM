...
@section('title', __('admin.portfolio.categories'))
@section('page_title', __('admin.portfolio.categories'))

@section('content')
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-extrabold text-slate-900 dark:text-white tracking-tight">{{ __('admin.portfolio.categories') }}</h1>
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">{{ __('admin.portfolio.summary_label') }}</p>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-xl h-fit">
            <h2 class="text-base font-bold text-slate-900 dark:text-white mb-4">{{ __('admin.portfolio.add_new') }} ({{ __('admin.portfolio.category_label') }})</h2>
            <form method="POST" action="{{ route('admin.portfolio.categories.store') }}" class="space-y-4">
                ...
                <div>
                    <label class="block text-xs font-semibold uppercase tracking-wider text-slate-700 dark:text-slate-300 mb-1.5">
                        {{ __('admin.portfolio.title_label') }} <span class="text-rose-400">*</span>
                    </label>
                    <input type="text" 
                           name="name" 
                           required 
                           placeholder="e.g. Cloud Infrastructure" 
                           class="w-full px-3.5 py-2 bg-slate-50 dark:bg-slate-950/60 border border-slate-300 dark:border-slate-800 rounded-xl text-slate-900 dark:text-white text-sm focus:ring-2 focus:ring-indigo-500">
                </div>
                ...
                <div>
                    <label class="block text-xs font-semibold uppercase tracking-wider text-slate-700 dark:text-slate-300 mb-1.5">
                        {{ __('admin.portfolio.summary_label') }}
                    </label>
                    <textarea name="description" 
                              rows="2" 
                              placeholder="Brief scope of projects in this category..." 
                              class="w-full px-3.5 py-2 bg-slate-50 dark:bg-slate-950/60 border border-slate-300 dark:border-slate-800 rounded-xl text-slate-900 dark:text-white text-sm focus:ring-2 focus:ring-indigo-500"></textarea>
                </div>
                ...
                <div class="pt-2">
                    <button type="submit" class="w-full py-2.5 rounded-xl text-white font-semibold bg-indigo-600 hover:bg-indigo-500 shadow-lg shadow-indigo-600/30 transition text-sm">
        ...
        <div class="lg:col-span-2 bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-xl">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-start text-sm text-slate-700 dark:text-slate-300">
                        <thead class="bg-slate-50 dark:bg-slate-950/60 text-[11px] font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 border-b border-slate-200 dark:border-slate-800">
                            <tr>
                                <th class="px-6 py-3.5">{{ __('admin.portfolio.title_label') }}</th>
                                <th class="px-6 py-3.5">Slug</th>
                                <th class="px-6 py-3.5">{{ __('admin.portfolio.table_category') }}</th>
                                <th class="px-6 py-3.5 text-end">{{ __('admin.portfolio.table_actions') }}</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-200 dark:divide-slate-800/60">
                            @foreach($categories as $category)
                                <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition" x-data="{ editing: false, name: '{{ addslashes($category->name) }}', slug: '{{ $category->slug }}', description: '{{ addslashes($category->description ?? '') }}', order: '{{ $category->order }}' }">
                                    <td class="px-6 py-4">
                                        <div x-show="!editing">
                                    ...
                                    <td class="px-6 py-4">
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-700">
                                            {{ $category->portfolios_count }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 text-end">
                                        <div class="flex items-center justify-end gap-2" x-show="!editing">
                                            <button @click="editing = true" class="p-1.5 text-slate-500 dark:text-slate-400 hover:text-indigo-400 hover:bg-slate-100 dark:bg-slate-800 rounded-lg transition" title="{{ __('ui.buttons.edit') }}" aria-label="{{ __('ui.buttons.edit') }}">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"/></svg>
                                            </button>
                                            <form method="POST" action="{{ route('admin.portfolio.categories.destroy', $category) }}" onsubmit="return confirm('{{ __('ui.confirmations.delete_category') }}');">
                                                @csrf
                                                @method('DELETE')
                                        ...
                                        <div class="flex items-center justify-end gap-2" x-show="editing" x-cloak>
                                            <form method="POST" action="{{ route('admin.portfolio.categories.update', $category) }}" class="inline-flex items-center gap-1">
                                                ...
                                                <button type="submit" class="px-2 py-1 bg-emerald-600 hover:bg-emerald-500 text-white rounded text-xs font-semibold">{{ __('ui.buttons.save') }}</button>
                                                <button type="button" @click="editing = false" class="px-2 py-1 bg-slate-100 dark:bg-slate-800 hover:bg-slate-700 text-slate-700 dark:text-slate-300 rounded text-xs">{{ __('ui.buttons.cancel') }}</button>
                                            </form>
                                        </div>
                                    </td>
                                </tr>
            ...
            @else
                <div class="py-12 px-4 text-center">
                    <p class="text-sm text-slate-500 dark:text-slate-400">{{ __('ui.empty_states.no_records_found') }}</p>
                </div>
            @endif
        </div>
    </div>
