...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-extrabold text-white tracking-tight">Portfolio & Case Studies</h1>
            <p class="text-sm text-slate-400 mt-1">Showcase client successes, architecture transformations, and technical stacks.</p>
        </div>
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.portfolio.categories') }}" class="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold text-slate-300 bg-slate-800 hover:bg-slate-700 border border-slate-700 transition">
    ...
    <div class="bg-slate-900/80 border border-slate-800 rounded-2xl p-4 shadow-xl">
        <form method="GET" action="{{ route('admin.portfolio.index') }}" class="grid grid-cols-1 sm:grid-cols-12 gap-3">
            ...
            <div class="sm:col-span-3">
                <select name="category_id" class="w-full px-3 py-2 bg-slate-950/60 border border-slate-800 rounded-xl text-white text-sm focus:ring-2 focus:ring-indigo-500">
    ...
    <div class="bg-slate-900/80 border border-slate-800 rounded-2xl overflow-hidden shadow-xl">
                            ...
                            <tr class="hover:bg-slate-800/30 transition">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        ...
                                        @endif
                                        <div>
                                            <a href="{{ route('admin.portfolio.edit', $portfolio) }}" class="font-semibold text-white hover:text-indigo-400 transition">
                                                {{ $portfolio->title }}
                                            </a>
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex flex-wrap gap-1 max-w-xs">
                                        ...
                                        @if(count($portfolio->technologies ?? []) > 3)
                                            <span class="text-[10px] text-slate-400">+{{ count($portfolio->technologies) - 3 }} more</span>
                                        @endif
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        <a href="{{ route('admin.portfolio.edit', $portfolio) }}" class="p-1.5 text-slate-400 hover:text-indigo-400 hover:bg-slate-800 rounded-lg transition" title="Edit">
        ...
        @else
            <div class="py-16 px-4 text-center">
                ...
                <h3 class="text-base font-bold text-white">No Case Studies Found</h3>
                <p class="text-xs text-slate-400 mt-1 max-w-sm mx-auto">Create and publish corporate case studies to demonstrate verifiable industry authority.</p>
                <div class="mt-4">
