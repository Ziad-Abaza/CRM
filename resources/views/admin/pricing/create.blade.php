...
<div class="max-w-4xl space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-extrabold text-slate-900 dark:text-white tracking-tight">Create Pricing Plan</h1>
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Configure plan parameters, feature deliverables, and WhatsApp engagement triggers.</p>
        </div>
    ...
    <form method="POST" action="{{ route('admin.pricing.store') }}" class="bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 sm:p-8 shadow-xl space-y-6">
        @csrf
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
            ...
            <div>
                ...
                <input type="number" 
                       ...
                       required 
                       class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-50 dark:bg-slate-950/60 border border-slate-300 dark:border-slate-200 dark:border-slate-800 rounded-xl text-slate-900 dark:text-white text-sm font-mono focus:ring-2 focus:ring-indigo-500 @error('price') border-rose-500 @enderror">
            ...
            <div>
                <label for="billing_period" class="block text-xs font-semibold uppercase tracking-wider text-slate-700 dark:text-slate-300 mb-1.5">
            ...
            <div class="sm:col-span-3">
                <label for="description" class="block text-xs font-semibold uppercase tracking-wider text-slate-700 dark:text-slate-300 mb-1.5">
                ...
                <textarea id="description" 
                          ...
                          class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-50 dark:bg-slate-950/60 border border-slate-300 dark:border-slate-200 dark:border-slate-800 rounded-xl text-slate-900 dark:text-white text-sm focus:ring-2 focus:ring-indigo-500">{{ old('description') }}</textarea>
            </div>

            <div class="sm:col-span-3">
                <label for="whatsapp_message" class="block text-xs font-semibold uppercase tracking-wider text-slate-700 dark:text-slate-300 mb-1.5">
        ...
        <div class="border-t border-slate-200 dark:border-slate-800 pt-6 space-y-4" x-data="{ items: {{ json_encode(old('features', ['Dedicated Lead Architect', 'Weekly Sprint Review', 'Priority 4-hr SLA SLA response', 'Unlimited Infrastructure Repositories'])) }} }">
            <div class="flex items-center justify-between">
                <div>
                    <h2 class="text-sm font-bold text-slate-900 dark:text-white uppercase tracking-wider">Plan Deliverables & Inclusions</h2>
                    <p class="text-xs text-slate-500 dark:text-slate-400">Features displayed as checkmarked bullets on the pricing card.</p>
                </div>
            ...
            <div class="space-y-2">
                <template x-for="(item, index) in items" :key="index">
                    <div class="flex items-center gap-2">
                        ...
                        <button type="button" @click="items.splice(index, 1)" class="p-2 text-slate-500 hover:text-rose-400 rounded-lg hover:bg-slate-100 dark:bg-slate-800 transition">
        ...
        <div class="border-t border-slate-200 dark:border-slate-800 pt-6 grid grid-cols-1 sm:grid-cols-3 gap-6">
            ...
            <div class="flex items-center pt-6">
                <label class="relative flex items-center gap-3 cursor-pointer">
                    ...
                    <span class="text-xs font-semibold uppercase tracking-wider text-slate-700 dark:text-slate-300">Highlight as Popular</span>
                </label>
            </div>
        ...
        <div class="pt-4 flex justify-end gap-3">
            <a href="{{ route('admin.pricing.index') }}" class="px-5 py-2.5 rounded-xl text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-white text-sm transition">Cancel</a>
