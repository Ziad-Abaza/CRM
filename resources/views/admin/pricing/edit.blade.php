...
<div class="max-w-4xl space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-extrabold text-white tracking-tight">Edit Pricing Plan</h1>
            <p class="text-sm text-slate-400 mt-1">Update deliverables and pricing metrics for "{{ $plan->name }}".</p>
        </div>
    ...
    <form method="POST" action="{{ route('admin.pricing.update', $plan) }}" class="bg-slate-900/80 border border-slate-800 rounded-2xl p-6 sm:p-8 shadow-xl space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
            ...
            <div class="sm:col-span-3">
                ...
                <textarea id="description" 
                          name="description" 
                          rows="2" 
                          class="w-full px-4 py-2.5 bg-slate-950/60 border border-slate-800 rounded-xl text-white text-sm focus:ring-2 focus:ring-indigo-500">{{ old('description', $plan->description) }}</textarea>
            </div>
        ...
        <div class="border-t border-slate-800 pt-6 space-y-4" x-data="{ items: {{ json_encode(old('features', $plan->features ?? [])) }} }">
            <div class="flex items-center justify-between">
                <div>
