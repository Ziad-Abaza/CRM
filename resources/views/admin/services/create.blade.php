...
<div class="max-w-4xl space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-extrabold text-white tracking-tight">Create Service</h1>
            <p class="text-sm text-slate-400 mt-1">Add a new strategic capability or consulting service.</p>
        </div>
    ...
    <form method="POST" action="{{ route('admin.services.store') }}" enctype="multipart/form-data" class="bg-slate-900/80 border border-slate-800 rounded-2xl p-6 sm:p-8 shadow-xl space-y-6">
        @csrf
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div class="sm:col-span-2">
                ...
                <textarea id="short_description" 
                          ...
                          class="w-full px-4 py-2.5 bg-slate-950/60 border border-slate-800 rounded-xl text-white text-sm focus:ring-2 focus:ring-indigo-500">{{ old('short_description') }}</textarea>
        ...
        <div class="border-t border-slate-800 pt-6 space-y-4" x-data="{ items: {{ json_encode(old('features', ['24/7 SLA Backing', 'Cloud Migration Matrix'])) }} }">
            <div class="flex items-center justify-between">
                <div>
        ...
        <div class="border-t border-slate-800 pt-6 grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div class="flex items-center pt-6">
                <label class="relative flex items-center gap-3 cursor-pointer">
                    ...
                    <span class="text-xs font-semibold uppercase tracking-wider text-slate-300">Publish Immediately</span>
                </label>
            </div>
        </div>
