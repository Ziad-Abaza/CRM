...
@section('page_title', 'Services / New')

@section('content')
<div class="max-w-4xl space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-extrabold text-slate-900 dark:text-white tracking-tight">Create Service</h1>
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Add a new strategic capability or consulting service.</p>
        </div>
    ...
    <form method="POST" action="{{ route('admin.services.store') }}" enctype="multipart/form-data" class="bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 sm:p-8 shadow-xl space-y-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div>
                <label for="icon" class="block text-xs font-semibold uppercase tracking-wider text-slate-700 dark:text-slate-300 mb-1.5">
                    Icon Identifier / SVG Name
                </label>
                <input type="text" 
                       id="icon" 
                       name="icon" 
                       value="{{ old('icon', 'sparkles') }}" 
                       placeholder="e.g. cloud, shield, cpu, bar-chart" 
                       class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-50 dark:bg-slate-950/60 border border-slate-300 dark:border-slate-200 dark:border-slate-800 rounded-xl text-slate-900 dark:text-white text-sm focus:ring-2 focus:ring-indigo-500">
            ...
            <div class="sm:col-span-2">
                ...
                <textarea id="short_description" 
                          ...
                          rows="2" 
                          placeholder="Brief 1-2 sentence executive overview of the service offering..." 
                          class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-50 dark:bg-slate-950/60 border border-slate-300 dark:border-slate-200 dark:border-slate-800 rounded-xl text-slate-900 dark:text-white text-sm focus:ring-2 focus:ring-indigo-500">{{ old('short_description') }}</textarea>
        ...
        <div class="border-t border-slate-200 dark:border-slate-800 pt-6 space-y-4" x-data="{ items: {{ json_encode(old('features', ['24/7 SLA Backing', 'Cloud Migration Matrix'])) }} }">
            <div class="flex items-center justify-between">
                <div>
        ...
        <div class="border-t border-slate-200 dark:border-slate-800 pt-6 grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div class="flex items-center pt-6">
                <label class="relative flex items-center gap-3 cursor-pointer">
                    ...
                    <span class="text-xs font-semibold uppercase tracking-wider text-slate-700 dark:text-slate-300">Publish Immediately</span>
                </label>
            </div>
        </div>
