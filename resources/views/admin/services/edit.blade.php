...
@section('title', __('admin.services.table_title') . ': ' . $service->title)
@section('page_title', __('admin.services.table_title'))

@php
    $titleTrans = $service->getTranslations('title');
    $shortDescTrans = $service->getTranslations('short_description');
    $descTrans = $service->getTranslations('description');
    $featuresTrans = $service->getTranslations('features');
    $featuresEn = is_array($featuresTrans) && isset($featuresTrans['en']) ? $featuresTrans['en'] : (is_array($service->features) ? $service->features : []);
...
<div class="max-w-4xl space-y-6" x-data="{ 
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-extrabold text-slate-900 dark:text-white tracking-tight">{{ __('ui.buttons.edit') }}: {{ $service->title }}</h1>
        ...
        <div x-show="langTab === 'en'" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" 
                       id="title_en" 
                       name="title[en]" 
                       value="{{ old('title.en', $titleTrans['en'] ?? $service->title) }}" 
                       required 
                       dir="ltr"
            ...
            <div>
                ...
                <textarea id="short_description_en" 
                          ...
                          rows="2" 
                          dir="ltr"
                          class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-950/60 border border-slate-300 dark:border-slate-800 rounded-xl text-slate-900 dark:text-white text-sm focus:ring-2 focus:ring-indigo-500">{{ old('short_description.en', $shortDescTrans['en'] ?? $service->short_description) }}</textarea>
            </div>
            ...
            <div>
                ...
                <textarea id="description_en" 
                          ...
                          rows="4" 
                          dir="ltr"
                          class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-950/60 border border-slate-300 dark:border-slate-800 rounded-xl text-slate-900 dark:text-white text-sm focus:ring-2 focus:ring-indigo-500">{{ old('description.en', $descTrans['en'] ?? $service->description) }}</textarea>
            </div>
        ...
        <div x-show="langTab === 'ar'" x-cloak class="space-y-4">
            ...
            <div>
                ...
                <textarea id="short_description_ar" 
                          ...
                          rows="2" 
                          dir="rtl"
                          class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-950/60 border border-slate-300 dark:border-slate-800 rounded-xl text-slate-900 dark:text-white text-sm focus:ring-2 focus:ring-indigo-500">{{ old('short_description.ar', $shortDescTrans['ar'] ?? '') }}</textarea>
            </div>
            ...
            <div>
                ...
                @if($service->image)
                    <div class="mb-3 flex items-center gap-3">
                        <img src="{{ $service->image }}" alt="{{ $service->title }}" class="w-14 h-14 rounded-xl object-cover border border-slate-700">
                    </div>
                @endif
                <input type="file" 
                       name="image" 
