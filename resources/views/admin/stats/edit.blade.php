...
<div class="max-w-3xl space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-extrabold text-white tracking-tight">Edit Metric Counter</h1>
            <p class="text-sm text-slate-400 mt-1">Update values for "{{ $stat->label }}".</p>
        </div>
    ...
    <form method="POST" action="{{ route('admin.stats.update', $stat) }}" class="bg-slate-900/80 border border-slate-800 rounded-2xl p-6 sm:p-8 shadow-xl space-y-6">
        @csrf
        @method('PUT')
