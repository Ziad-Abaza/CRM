...
@section('title', __('ui.buttons.edit') . ': ' . $member->name)
@section('page_title', __('admin.team.title'))

@php
    $nameTrans = $member->getTranslations('name');
    $roleTrans = $member->getTranslations('role');
    $bioTrans = $member->getTranslations('bio');
@endphp
...
<div class="max-w-4xl space-y-6" x-data="{ langTab: 'en' }">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-extrabold text-slate-900 dark:text-white tracking-tight">{{ __('ui.buttons.edit') }}: {{ $member->name }}</h1>
        ...
        <div x-show="langTab === 'en'" class="space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" 
                           id="name_en" 
                           name="name[en]" 
                           value="{{ old('name.en', $nameTrans['en'] ?? $member->name) }}" 
                           required 
                           dir="ltr"
                ...
                <div>
                    ...
                    <input type="text" 
                           id="role_en" 
                           name="role[en]" 
                           value="{{ old('role.en', $roleTrans['en'] ?? $member->role) }}" 
                           required 
                           dir="ltr"
                           class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-950/60 border border-slate-300 dark:border-slate-800 rounded-xl text-slate-900 dark:text-white text-sm focus:ring-2 focus:ring-indigo-500">
                </div>
            ...
            <div>
                ...
                <textarea id="bio_en" 
                          ...
                          rows="3" 
                          dir="ltr"
                          class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-950/60 border border-slate-300 dark:border-slate-800 rounded-xl text-slate-900 dark:text-white text-sm focus:ring-2 focus:ring-indigo-500">{{ old('bio.en', $bioTrans['en'] ?? $member->bio) }}</textarea>
            </div>
        </div>
        ...
        <div x-show="langTab === 'ar'" x-cloak class="space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="text" 
                           id="role_ar" 
                           name="role[ar]" 
                           value="{{ old('role.ar', $roleTrans['ar'] ?? '') }}" 
                           required 
                           dir="rtl"
                           class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-950/60 border border-slate-300 dark:border-slate-800 rounded-xl text-slate-900 dark:text-white text-sm focus:ring-2 focus:ring-indigo-500">
                </div>
            ...
            <div>
                ...
                <textarea id="bio_ar" 
                          ...
                          rows="3" 
                          dir="rtl"
                          class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-950/60 border border-slate-300 dark:border-slate-800 rounded-xl text-slate-900 dark:text-white text-sm focus:ring-2 focus:ring-indigo-500">{{ old('bio.ar', $bioTrans['ar'] ?? '') }}</textarea>
            </div>
        </div>
            ...
            <div>
                ...
                @if($member->photo)
                    <div class="mb-3 flex items-center gap-3">
                        <img src="{{ $member->photo }}" alt="{{ $member->name }}" class="w-12 h-12 rounded-full object-cover border border-slate-700">
                    </div>
                @endif
                <input type="file" 
                       name="photo" 
