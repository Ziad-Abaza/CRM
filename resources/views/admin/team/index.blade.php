...
@section('page_title', __('admin.nav.team'))

@section('content')
<div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-extrabold text-slate-900 dark:text-white tracking-tight">{{ __('admin.team.title') }}</h1>
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">{{ __('admin.team.role_label') }}</p>
        </div>
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.team.create') }}" class="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold text-white bg-indigo-600 hover:bg-indigo-500 shadow-lg shadow-indigo-600/20 transition">
                ...
                <span>{{ __('admin.team.add_new') }}</span>
            </a>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-xl">
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-start text-sm text-slate-700 dark:text-slate-300">
                    <thead class="bg-slate-50 dark:bg-slate-950/60 text-[11px] font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 border-b border-slate-200 dark:border-slate-800">
                        <tr>
                            <th class="px-6 py-3.5">{{ __('admin.team.order_label') }}</th>
                            <th class="px-6 py-3.5">{{ __('admin.team.table_title') }}</th>
                            <th class="px-6 py-3.5">{{ __('admin.team.table_role') }}</th>
                            <th class="px-6 py-3.5">{{ __('admin.team.table_contact') }}</th>
                            <th class="px-6 py-3.5">{{ __('admin.team.table_status') }}</th>
                            <th class="px-6 py-3.5 text-end">{{ __('admin.team.table_actions') }}</th>
                        </tr>
                    </thead>
                            ...
                            <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        ...
                                        @else
                                            <div class="w-10 h-10 rounded-full bg-slate-100 dark:bg-slate-800 border border-slate-700 text-slate-700 dark:text-slate-300 flex items-center justify-center font-bold text-xs">
                                                {{ mb_substr($member->name, 0, 1) }}
                                            </div>
                                        @endif
                                        <div>
                                            <a href="{{ route('admin.team.edit', $member) }}" class="font-semibold text-slate-900 dark:text-white hover:text-indigo-400 transition">
                                                {{ $member->name }}
                                            </a>
                                ...
                                <td class="px-6 py-4">
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-slate-100 dark:bg-slate-800 text-indigo-300 border border-slate-700">
                                        {{ $member->role }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 text-xs space-y-1 font-mono text-slate-500 dark:text-slate-400">
                                    @if($member->email)
                                ...
                                <td class="px-6 py-4">
                                    <form method="POST" action="{{ route('admin.team.toggle', $member) }}">
                                        ...
                                        <button type="submit" class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold {{ $member->is_active ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-rose-500/10 text-rose-400 border border-rose-500/20' }}">
                                            ...
                                            <span>{{ $member->is_active ? __('ui.filter.active') : __('ui.filter.inactive') }}</span>
                                        </button>
                                    </form>
                                </td>
                                <td class="px-6 py-4 text-end">
                                    <div class="flex items-center justify-end gap-2">
                                        <a href="{{ route('admin.team.edit', $member) }}" class="p-1.5 text-slate-500 dark:text-slate-400 hover:text-indigo-400 hover:bg-slate-100 dark:bg-slate-800 rounded-lg transition" title="{{ __('ui.buttons.edit') }}" aria-label="{{ __('ui.buttons.edit') }}">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"/></svg>
                                        </a>
                                        <form method="POST" action="{{ route('admin.team.destroy', $member) }}" onsubmit="return confirm('{{ __('ui.confirmations.delete_team_member') }}');">
                                            @csrf
                                            @method('DELETE')
        ...
        @else
            <div class="py-16 px-4 text-center">
                ...
                <p class="text-xs text-slate-500 dark:text-slate-400 mt-1 max-w-sm mx-auto">{{ __('admin.team.role_label') }}</p>
                <div class="mt-4">
