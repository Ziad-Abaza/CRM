...
<div class="max-w-4xl space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-extrabold text-white tracking-tight">Edit Client Testimonial</h1>
            <p class="text-sm text-slate-400 mt-1">Update testimonial details for "{{ $testimonial->client_name }}".</p>
        </div>
        <a href="{{ route('admin.testimonials.index') }}" class="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-xl text-xs font-semibold transition">
            &larr; Back to Testimonials
        </a>
    </div>

    <form method="POST" action="{{ route('admin.testimonials.update', $testimonial) }}" enctype="multipart/form-data" class="bg-slate-900/80 border border-slate-800 rounded-2xl p-6 sm:p-8 shadow-xl space-y-6">
        @csrf
        @method('PUT')

        <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
            <div>
                <label for="client_name" class="block text-xs font-semibold uppercase tracking-wider text-slate-300 mb-1.5">
                ...
                <input type="text" 
                       ...
                       required 
                       class="w-full px-4 py-2.5 bg-slate-950/60 border border-slate-800 rounded-xl text-white text-sm focus:ring-2 focus:ring-indigo-500 @error('client_name') border-rose-500 @enderror">
                @error('client_name') <p class="mt-1 text-xs text-rose-400">{{ $message }}</p> @enderror
            </div>

            <div>
                <label for="client_role" class="block text-xs font-semibold uppercase tracking-wider text-slate-300 mb-1.5">
                    Job Title / Role
                </label>
                <input type="text" 
                       id="client_role" 
            ...
            <div>
                <label for="company" class="block text-xs font-semibold uppercase tracking-wider text-slate-300 mb-1.5">
                    Company Name
                </label>
                <input type="text" 
                       id="company" 
            ...
            <div>
                <label for="rating" class="block text-xs font-semibold uppercase tracking-wider text-slate-300 mb-1.5">
                    Star Rating (1-5) <span class="text-rose-400">*</span>
                </label>
                <select name="rating" id="rating" class="w-full px-4 py-2.5 bg-slate-950/60 border border-slate-800 rounded-xl text-white text-sm focus:ring-2 focus:ring-indigo-500">
            ...
            <div class="sm:col-span-2">
                ...
                @if($testimonial->avatar)
                    <div class="mb-3 flex items-center gap-3">
                        <img src="{{ $testimonial->avatar }}" alt="Avatar" class="w-12 h-12 rounded-full object-cover border border-slate-700">
            ...
            <div class="sm:col-span-3">
                ...
                <textarea id="content" 
                          ...
                          rows="4" 
                          required 
                          class="w-full px-4 py-2.5 bg-slate-950/60 border border-slate-800 rounded-xl text-white text-sm focus:ring-2 focus:ring-indigo-500 @error('content') border-rose-500 @enderror">{{ old('content', $testimonial->content) }}</textarea>
        ...
        <div class="border-t border-slate-800 pt-6 grid grid-cols-1 sm:grid-cols-3 gap-6">
            ...
            <div class="flex items-center pt-6">
                <label class="relative flex items-center gap-3 cursor-pointer">
                    ...
                    <span class="text-xs font-semibold uppercase tracking-wider text-slate-300">Feature on Home Carousel</span>
                </label>
            </div>
        ...
        <div class="pt-4 flex justify-end gap-3">
            <a href="{{ route('admin.testimonials.index') }}" class="px-5 py-2.5 rounded-xl text-slate-400 hover:text-white text-sm transition">Cancel</a>
