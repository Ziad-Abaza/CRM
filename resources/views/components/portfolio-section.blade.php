        ...
        <div class="max-w-3xl mx-auto text-center space-y-3 sm:space-y-4 mb-8 sm:mb-12">
            ...
            <div class="flex flex-wrap items-center justify-center gap-2 pt-2">
                <button type="button" 
                    ...
                    {{ __('frontend.portfolio.all_categories') }}
                </button>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6">
            @foreach($portfolios as $portfolio)
                <div x-show="selectedCategory === 'all' || selectedCategory === {{ $portfolio->category_id ?? 0 }}"
                     ...
                     class="rounded-2xl border border-slate-200 dark:border-slate-800/90 bg-white dark:bg-slate-900/90 shadow-sm hover:shadow-xl hover:border-emerald-500/40 transition duration-300 flex flex-col justify-between overflow-hidden group text-start">
                    ...
                    <div class="relative h-36 sm:h-40 overflow-hidden bg-slate-100 dark:bg-slate-950">
                        ...
                        <!-- Category Pill -->
                        <span class="absolute top-3 start-3 px-2.5 py-0.5 rounded-full bg-white/90 dark:bg-slate-950/80 backdrop-blur-md border border-slate-200 dark:border-slate-800 text-[10px] font-bold text-emerald-700 dark:text-emerald-300 uppercase tracking-wider">
                    ...
                    <div class="p-5 sm:p-6 space-y-3 flex-1 flex flex-col justify-between">
                        <div class="space-y-2">
                            <span class="text-[11px] font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">{{ __('frontend.portfolio.client') }}: {{ $portfolio->client }}</span>
                            <h3 class="text-base sm:text-lg font-bold text-slate-900 dark:text-white leading-snug group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition">
                                <a href="{{ localized_route('portfolio.detail', $portfolio->slug) }}">
                                    {{ $portfolio->title }}
                                </a>
                            </h3>
                        ...
                        @if(!empty($portfolio->technologies))
                            <div class="flex flex-wrap gap-1.5 pt-3 border-t border-slate-100 dark:border-slate-800" dir="ltr">
                    ...
                    <div class="px-5 sm:px-6 py-3.5 bg-slate-50 dark:bg-slate-950/60 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                        <a href="{{ localized_route('portfolio.detail', $portfolio->slug) }}" class="text-xs font-bold text-emerald-600 dark:text-emerald-400 hover:text-emerald-700 dark:hover:text-emerald-300 flex items-center gap-1">
