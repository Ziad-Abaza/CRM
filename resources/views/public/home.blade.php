...
@section('content')
    ...
    <x-portfolio-section :categories="$categories" :portfolioItems="$portfolioItems" />

    <!-- 5. About & Executive Capabilities Section -->
    <section id="about" class="py-20 lg:py-32 bg-slate-950 relative border-b border-slate-800/80">
            ...
            <div class="grid lg:grid-cols-12 gap-12 lg:gap-16 items-center">
                    ...
                    <div class="space-y-4 pt-4">
                        @if(setting('about_bullet_1'))
                            <div class="flex items-start gap-3">
                                <div class="h-6 w-6 rounded-lg bg-emerald-500/10 border border-emerald-500/30 flex items-center justify-center text-emerald-400 text-xs font-bold mt-0.5 flex-shrink-0">
                                    ✓
                                </div>
                                <p class="text-sm font-medium text-slate-200">{{ setting('about_bullet_1') }}</p>
                            </div>
                        @endif
                        ...
                        @if(setting('about_bullet_2'))
                            <div class="flex items-start gap-3">
                                ...
                                <p class="text-sm font-medium text-slate-200">{{ setting('about_bullet_2') }}</p>
                            </div>
                        @endif
                        ...
                        @if(setting('about_bullet_3'))
                            <div class="flex items-start gap-3">
                                ...
                                <p class="text-sm font-medium text-slate-200">{{ setting('about_bullet_3') }}</p>
                            </div>
                        @endif
                    </div>
                ...
                <div class="lg:col-span-6">
                    <div class="grid grid-cols-2 gap-4">
                        <div class="space-y-4">
                            <div class="p-6 rounded-3xl bg-slate-900/90 border border-slate-800 space-y-2">
                                <div class="text-2xl">⚡</div>
                                <h3 class="text-base font-bold text-white">High-Throughput</h3>
                                <p class="text-xs text-slate-400">Zero-latency distributed infrastructure for financial workflows.</p>
                            </div>
                            <div class="p-6 rounded-3xl bg-slate-900/90 border border-slate-800 space-y-2">
                                <div class="text-2xl">🛡️</div>
                                <h3 class="text-base font-bold text-white">Full Governance</h3>
                                <p class="text-xs text-slate-400">SOC 2 Type II, ISO 27001, and HIPAA automated audit frameworks.</p>
                            </div>
                        </div>
                        <div class="space-y-4 pt-6">
                            <div class="p-6 rounded-3xl bg-slate-900/90 border border-slate-800 space-y-2">
                                <div class="text-2xl">🤝</div>
                                <h3 class="text-base font-bold text-white">Direct Partner SLA</h3>
                                <p class="text-xs text-slate-400">Direct WhatsApp executive access with &lt;15 minute SLA.</p>
                            </div>
                            <div class="p-6 rounded-3xl bg-slate-900/90 border border-slate-800 space-y-2">
                                <div class="text-2xl">📈</div>
                                <h3 class="text-base font-bold text-white">Verified ROI</h3>
                                <p class="text-xs text-slate-400">Measurable cost reduction and cycle compression in 90 days.</p>
                            </div>
                        </div>
                    </div>
                </div>
