...
@section('content')
    ...
    <section class="py-10 sm:py-14 lg:py-16 bg-white dark:bg-slate-950 bg-grid-pattern relative border-b border-slate-200 dark:border-slate-800/80 transition-colors duration-200">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="max-w-4xl space-y-4 sm:space-y-5 text-start">
                <!-- Breadcrumbs -->
                <nav class="flex items-center gap-1.5 sm:gap-2 text-xs font-semibold text-slate-500 dark:text-slate-400">
                    ...
                    <span>/</span>
                    <a href="{{ localized_route('home') }}#portfolio" class="hover:text-slate-900 dark:hover:text-white transition">{{ __('ui.nav.case_studies') }}</a>
                    <span>/</span>
                ...
                <div class="flex flex-wrap items-center gap-2 sm:gap-3">
                    <span class="px-2.5 py-0.5 rounded-full bg-emerald-50 dark:bg-emerald-950/80 border border-emerald-200 dark:border-emerald-500/30 text-emerald-700 dark:text-emerald-300 text-xs font-semibold uppercase tracking-wider">
                        {{ $portfolio->category?->name ?? __('frontend.portfolio.section_badge') }}
                    </span>
                    <span class="text-xs font-semibold text-slate-500 dark:text-slate-400">{{ __('frontend.portfolio.client') }}: {{ $portfolio->client }}</span>
                </div>
                ...
                <div class="pt-1 flex flex-wrap items-center gap-3 sm:gap-4">
                    ...
                    @if($portfolio->website_url)
                        <a href="{{ $portfolio->website_url }}" target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-1.5 px-4 py-2 sm:py-2.5 rounded-xl bg-slate-100 dark:bg-slate-900 hover:bg-slate-200 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 text-xs sm:text-sm font-semibold border border-slate-200 dark:border-slate-800 transition">
                            <span>{{ current_locale() === 'ar' ? 'معاينة حية للمشروع' : 'Live Reference' }}</span>
            ...
            <div class="grid lg:grid-cols-12 gap-8 lg:gap-12 text-start">
                ...
                <div class="lg:col-span-8 space-y-8">
                    ...
                    <div class="space-y-3">
                        <h2 class="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white tracking-tight">
                            {{ __('frontend.portfolio.challenge') }} &amp; {{ __('frontend.portfolio.solution') }}
                        </h2>
                    ...
                    @if(!empty($portfolio->technologies))
                        <div class="space-y-3 pt-4 border-t border-slate-200 dark:border-slate-800/80">
                            <h3 class="text-base sm:text-lg font-bold text-slate-900 dark:text-white tracking-tight">
                                {{ __('frontend.portfolio.tech_stack') }}
                            </h3>
                    ...
                    <div class="p-5 sm:p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-4">
                        <div>
                            <h4 class="text-base font-bold text-slate-900 dark:text-white">{{ current_locale() === 'ar' ? 'هل تتطلب مؤسستكم كفاءة تشغيلية مماثلة؟' : 'Require similar institutional velocity?' }}</h4>
                            <p class="text-xs text-slate-600 dark:text-slate-300 mt-0.5">{{ current_locale() === 'ar' ? 'يمكن لفريقنا الاستشاري والهندسي تقييم متطلباتكم مباشرة عبر تطبيق واتساب.' : 'Our engineering team can evaluate your specifications directly on WhatsApp.' }}</p>
                        </div>
                        <x-whatsapp-cta-button 
                ...
                <div class="lg:col-span-4 space-y-6">
                    <div class="rounded-2xl bg-white dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 p-5 sm:p-6 space-y-4 sticky top-24 backdrop-blur-xl shadow-lg shadow-slate-200/50 dark:shadow-slate-950">
                        <h3 class="text-base font-bold text-slate-900 dark:text-white border-b border-slate-100 dark:border-slate-800 pb-3">{{ current_locale() === 'ar' ? 'معايير ومعطيات المشروع' : 'Project Parameters' }}</h3>
                        
                        <div class="space-y-3 text-xs">
                            <div>
                                <span class="text-slate-500 dark:text-slate-400 block font-medium mb-0.5">{{ __('frontend.portfolio.client') }}</span>
                                <span class="text-slate-900 dark:text-white font-bold text-sm">{{ $portfolio->client }}</span>
                            </div>

                            <div>
                                <span class="text-slate-500 dark:text-slate-400 block font-medium mb-0.5">{{ __('frontend.portfolio.industry') }}</span>
                                <span class="text-slate-900 dark:text-white font-semibold">{{ $portfolio->category?->name ?? __('frontend.portfolio.section_badge') }}</span>
                            </div>

                            @if($portfolio->completion_date)
                                <div>
                                    <span class="text-slate-500 dark:text-slate-400 block font-medium mb-0.5">{{ __('frontend.portfolio.timeframe') }}</span>
                                    <span class="text-slate-900 dark:text-white font-semibold">{{ $portfolio->completion_date->translatedFormat('F Y') }}</span>
                                </div>
                            @endif

                            <div>
                                <span class="text-slate-500 dark:text-slate-400 block font-medium mb-0.5">{{ current_locale() === 'ar' ? 'التدقيق والحوكمة' : 'Audit & Governance' }}</span>
                                <span class="text-emerald-600 dark:text-emerald-400 font-semibold flex items-center gap-1.5">
                                    <span class="w-1.5 h-1.5 rounded-full bg-emerald-500 flex-shrink-0"></span>
                                    {{ current_locale() === 'ar' ? 'موثق وجاهز للإنتاج' : 'Production Verified' }}
                                </span>
                            </div>
                        </div>
        ...
        <section class="py-10 sm:py-14 bg-white dark:bg-slate-950 bg-grid-pattern border-t border-slate-200 dark:border-slate-800/80 transition-colors duration-200">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-start">
                <div class="flex items-center justify-between mb-6">
                    <h3 class="text-lg sm:text-xl font-bold text-slate-900 dark:text-white">{{ __('frontend.portfolio.similar_projects') }}</h3>
                    <a href="{{ localized_route('home') }}#portfolio" class="text-xs font-bold text-blue-600 dark:text-blue-400 hover:underline">{{ __('ui.buttons.view_all') }}</a>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                    @foreach($relatedCaseStudies as $rel)
                        <div class="p-5 rounded-2xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:border-blue-500/40 transition duration-200 flex flex-col justify-between">
                            <div class="space-y-2">
                                <span class="text-[10px] font-semibold text-blue-600 dark:text-blue-400 uppercase tracking-wider">{{ $rel->category?->name ?? __('frontend.portfolio.section_badge') }}</span>
                                <h4 class="font-bold text-slate-900 dark:text-white text-sm sm:text-base">
                                    <a href="{{ localized_route('portfolio.detail', $rel->slug) }}" class="hover:text-blue-600 dark:hover:text-blue-400 transition">
                                        {{ $rel->title }}
                                    </a>
                                </h4>
                            ...
                            <div class="pt-3 mt-3 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between">
                                <a href="{{ localized_route('portfolio.detail', $rel->slug) }}" class="text-xs font-bold text-blue-600 dark:text-blue-400 hover:underline flex items-center gap-1">
                                    <span>{{ __('frontend.portfolio.view_study') }}</span>
                                    <svg class="w-3.5 h-3.5 rtl:rotate-180" fill="none" stroke="currentColor" viewBox="0 0 24 24">
