        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="max-w-4xl space-y-4 sm:space-y-5">
                ...
                <nav class="flex items-center gap-1.5 sm:gap-2 text-xs font-semibold text-slate-400">
                    ...
                    <span>/</span>
                    <span class="text-blue-400 truncate">{{ $portfolio->client }}</span>
                </nav>

                <div class="flex flex-wrap items-center gap-2 sm:gap-3">
                    <span class="px-2.5 py-0.5 rounded-full bg-emerald-950/80 border border-emerald-500/30 text-emerald-300 text-xs font-semibold uppercase tracking-wider">
                ...
                <div class="pt-1 flex flex-wrap items-center gap-3 sm:gap-4">
                    ...
                    @if($portfolio->website_url)
                        <a href="{{ $portfolio->website_url }}" target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-1.5 px-4 py-2 sm:py-2.5 rounded-xl bg-slate-900 hover:bg-slate-800 text-slate-300 text-xs sm:text-sm font-semibold border border-slate-800 transition">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid lg:grid-cols-12 gap-8 lg:gap-12">
                ...
                <div class="lg:col-span-8 space-y-8">
                    ...
                    @if($portfolio->image)
                        <div class="rounded-2xl overflow-hidden border border-slate-800 shadow-xl bg-slate-900">
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="text-slate-300 leading-relaxed text-sm sm:text-base space-y-3 whitespace-pre-line">
                            {{ $portfolio->content }}
                        </div>
                    </div>
                        ...
                        <div class="space-y-3 pt-4 border-t border-slate-800/80">
                            ...
                            <div class="flex flex-wrap gap-2">
                                @foreach($portfolio->technologies as $tech)
                                    <span class="px-2.5 py-1 rounded-lg bg-slate-900 border border-slate-800 text-xs font-mono text-blue-300">
                                        {{ $tech }}
                                    </span>
                                @endforeach
                            </div>
                    ...
                    <!-- Conversion Box -->
                    <div class="p-5 sm:p-6 rounded-2xl bg-gradient-to-r from-slate-900 to-slate-950 border border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-4">
                        <div>
                            <h4 class="text-base font-bold text-white">Require similar institutional velocity?</h4>
                            <p class="text-xs text-slate-300 mt-0.5">Our engineering team can evaluate your specifications directly on WhatsApp.</p>
                        </div>
                        <x-whatsapp-cta-button 
                ...
                <div class="lg:col-span-4 space-y-6">
                    <div class="rounded-2xl bg-slate-900/90 border border-slate-800 p-5 sm:p-6 space-y-4 sticky top-24 backdrop-blur-xl shadow-xl">
                        <h3 class="text-base font-bold text-white border-b border-slate-800 pb-3">Project Parameters</h3>
        ...
        <section class="py-10 sm:py-14 bg-slate-950 bg-grid-pattern border-t border-slate-800/80">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex items-center justify-between mb-6">
                    <h3 class="text-lg sm:text-xl font-bold text-white">More Enterprise Case Studies</h3>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                    @foreach($relatedCaseStudies as $rel)
                        <div class="p-5 rounded-2xl bg-slate-900 border border-slate-800 hover:border-blue-500/40 transition duration-200 flex flex-col justify-between">
                            <div class="space-y-2">
                                <span class="text-[10px] font-semibold text-blue-400 uppercase tracking-wider">{{ $rel->category?->name ?? 'Case Study' }}</span>
